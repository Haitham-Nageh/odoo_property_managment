...
## Enterprise Migration Audit (2026-09-24) - Discovery/Baseline Session (NO source changes, NOT committed)

**Environment (verified):** Odoo 19.0 FINAL **Enterprise** (enterprise addons merged into `server/odoo/addons`, 1472 dirs; `account_accountant`, `web_enterprise`, `web_studio`, `documents`, etc. present). Python 3.12.3 (`<root>/python`), PostgreSQL 16.14 on **5433** (5432 is a separate PG). Dev config `<root>/config/odoo-enterprise-dev.conf`: db `odoo19_enterprise_dev`, HTTP 8070, workers=0, `dbfilter=^odoo19_enterprise_dev$` (tests on other DBs need `--db-filter`). Launch: `config/run-enterprise-dev.cmd`. Use PowerShell (not Git-Bash) to pass `--test-tags /property_managment` - Git-Bash rewrites the leading `/`. `server/odoo.conf` is a stale template (points at `c:\program files\...`); harmless warning `option addons_path, no such directory` appears on every start.
**Dev DB:** 77 modules installed (incl. account_accountant, web_enterprise, web_gantt/grid/map, spreadsheet_*), `generic_coa`, single company (now "EDARA Real Estate Co.", currency changed USD->**ILS**; ILS/JOD/USD activated, rates on 2026-01-01: USD 0.27, JOD 0.19). Only `admin` existed (password reset locally for test infra; not recorded in this repo).
**Module:** `property_managment` 19.0.1.0.0 installed; deps `base, mail, portal, account` (no Enterprise dependency); 22 ir.model, 60 views, 24 menus, 23 act_window, 10 server actions, 9 crons, 46 ir.rule, 50 ACL rows (UI inventory doc says 43/47 - minor doc drift). No module-related WARNING/ERROR in install or server logs.
**Automated tests (fresh install, throwaway DB, `-i account_accountant,property_managment --without-demo=all`, PowerShell launch):** Community baseline 388/388 -> Enterprise **387/388** (1 failed, 0 errors, 598 s). Only failure: `TestEdaraBranch.test_branch_code_unique_per_company` - **test-fixture problem, not a regression**: after Phase 6.4 renamed the fixture branch to `TRAM`, this test relies on a *pre-existing* `RAM` branch that only existed in the old Community dev DB; on a clean DB nothing collides. Constraint itself verified enforced live. Test NOT patched.
**Live data built in dev DB (kept):** branches RAM/NAB; properties Al-Amal Towers (RAM, 1 owner 100%), Rafidia Plaza (NAB, 60/40 owners; >100% rejected); buildings TA/TB/PM; units 101,102,103,201,202,203,B01,P01; tenants (Nour Hamdan, Omar Jaber, Layla Saleh, Basel Trading Co., Rami Odeh); vendor Al-Salam Plumbing; contracts LC/2026/0006..0015 covering monthly/quarterly/yearly, ILS/USD/JOD, draft/active/reserved/terminated/expired/renewed; users `mgr.ram@`, `mgr.nab@`, `viewer.ram@`, `accountant@`, `company.mgr@edara.test` (local-only test passwords, not recorded here), portal `nour@`/`omar@tenant.test`.
**Verified working on Enterprise:** schedule math (30-day proration, day-31 anchor, leap day, JOD 3-dec rounding); rent invoices (partner/company/currency/sale journal/receivable/income acct/analytic 100%/1:1 schedule link, idempotent re-run); native payment register (full, partial, overpayment -> tenant credit 500 offered on open invoice, USD-in-USD, ILS payment on JOD invoice); late fee; deposit collect/deduct/refund (liability nets to 0, tx delete blocked); service charge invoices; maintenance vendor bill create/post/pay; all 9 crons; dashboard/financial summaries/reports reconcile with analytic lines and journal balance (0.00); 18 EDARA window actions load all views; `edara_payment_schedule_filter_shortcuts` registry key + template served (Enterprise bundle is `web.assets_web`), no `registerTemplateExtension("web.ListView")` from EDARA; portal isolation (403 on other tenant's lease, RPC AccessError, own-invoice scope) for two real tenant users; branch/viewer isolation and elevation gates for viewer.
**Not testable this session:** real browser click-through/console (Chrome extension not connected) - shortcut buttons verified only via served bundle, view arch and search-filter domains, not visually.

### Defects / observations found (documented only, NOT fixed)
1. **HIGH (integrity/security, RPC-only):** system-managed and stored-related fields are writable through ORM/RPC by a Branch Manager (UI hides them via readonly only): `edara.unit.branch_id` (transplants a unit to another branch, DB then inconsistent with its building/property, unit vanishes from the caller), `edara.lease.contract.company_id` (cross-company), `edara.payment.schedule.line.amount/branch_id/invoice_id` on an INVOICED line (defeats the Phase 13 audit-trail guard), `edara.lease.contract.state` (bypasses action_terminate etc.). All reproduced in rolled-back savepoints as `mgr.ram@edara.test`.
2. **MEDIUM (currency):** `edara.maintenance.request.action_create_vendor_bill()` never sets `currency_id`; a USD request (cost 200) yields an **ILS 200** bill.
3. **MEDIUM (analytics):** vendor bills carry EDARA tags but **no analytic_distribution** (rent/late-fee/service-charge invoices do), so maintenance expense is absent from the property analytic account.
4. **MEDIUM (billing gap, needs business decision):** `_process_due_invoices` skips any contract not `active`; after an *early* renewal (`action_renew` flips old contract to `renewed` immediately) the old contract's remaining uninvoiced periods are never billed, and the unit shows `reserved` until the successor starts.
5. **LOW:** deprecated-directive warning `t-raw` in mail templates is Odoo core, not EDARA. `.gitignore` cosmetic defect (pre-existing).
6. Design note (unchanged): no EDARA group implies a native Accounting group; branch manager/accountant/company manager all have `account.move` read = False; only Odoo admin sees native accounting screens.

### Next Internal Action
Await user decision on next phase. Recommended: (a) hardening ticket for defect 1 (make system-managed/related fields non-writable at ORM level or constrain in `write()`), (b) fix defects 2-3, (c) decide defect 4, (d) fix `test_branch_code_unique_per_company` fixture to create its own `RAM`-coded branch first, (e) browser click-through of Payment Schedule shortcuts in Enterprise UI.


## Phase 7 - Enterprise Hardening & Security (2026-09-24)

**Status: Implemented, tested, verified live (RPC + real browser). Enterprise baseline before: 387/388 (1 fixture failure). After: 403/403, 0 failed, 0 errors (388 existing + 15 new). Fresh install AND upgrade paths both green on Odoo 19 Enterprise.**

### Defect 1 (HIGH) - system-managed fields writable via ORM/RPC
- **Root cause:** stored `related` fields (branch_id/company_id/property_id/...) and lifecycle fields were protected only by view `readonly`; `write()` from RPC accepted them (a Branch Manager could move a unit to another branch, change a contract's company, force `state`, rewrite an invoiced line's amount/invoice_id).
- **Fix:** new `models/edara_system_field_guard.py` - abstract mixin `edara.system.field.guard` attached (without editing their files) to all 13 EDARA business models. It rejects `write()` of (a) every stored readonly related field and (b) explicit `_edara_system_fields` (`edara.lease.contract.state`, `edara.payment.schedule.line.invoice_id`) unless `env.su`; on `create()` client-supplied derived values are discarded (always recomputed). `env.su` is the authorization signal on purpose - a context flag would be client-forgeable via RPC (verified: `context={'su': True, ...}` is rejected). `edara.payment.schedule.line.write` additionally freezes `amount` once `invoice_id` is set (uninvoiced amounts stay editable - the views intentionally allow it).
- **Internal flows kept working** via `check_access('write')` first + narrowly scoped `.sudo()` write: `action_activate`, `action_terminate`, `action_renew`, `_cron_expire_contracts` (contract `state`), `_create_invoice` (`invoice_id`). The added `check_access('write')` is REQUIRED: without it the sudo would let a read-only Viewer drive the lifecycle (tested).
- **Not covered / intentionally unchanged:** `state` of other models (renewal request, maintenance request, service charge...) and plain (non-related) audit fields such as `due_date`/`period_*` on invoiced lines - outside the audited scope; candidates for a follow-up.

### Defect 2 (MEDIUM) - maintenance vendor bill currency
- Intended rule (from the model): `cost` is a Monetary in the request's own `currency_id` (defaults to company currency), so the bill takes `currency_id` from the request. `action_create_vendor_bill` now sets it. Verified ILS/USD/JOD (bill currency, amount, purchase journal, vendor, company, expense account, post, native payment; USD 200 -> 740.74 ILS at rate 0.27).

### Defect 3 (MEDIUM) - maintenance vendor bill analytics
- Bill line now carries `analytic_distribution = {property analytic account: 100}` using the existing `edara.property.get_analytic_account()` (same mechanism as rent/late fee/service charge). Verified: analytic line created on post (amount in company currency), payment reconciles natively; an unrelated vendor bill gets no distribution.

### Defect 4 (MEDIUM) - early renewal billing - BUSINESS DECISION
- **Existing semantics (verified from code + MAT history):** the renewal wizard defaults the successor's start to the predecessor's end date; MAT-024 renewed a contract a year ahead with the successor starting at the old end. So the predecessor is marked `renewed` immediately but **stays financially responsible for its own term**; the successor only covers its own term. (The "old contract stops being billable" hypothesis would create a revenue gap and was rejected.)
- **Fix:** `_process_due_invoices` now bills `renewed` contracts as well as `active` (terminated/expired unchanged). `action_renew` drops only the predecessor's uninvoiced lines with `period_start >= successor start` (overlapping renewal => no double billing).
- **Live evidence:** LC/2026/0012 (renewed) had 9 due-but-never-invoiced lines; after the fix they were invoiced, future lines (Oct-Dec) stay uninvoiced until due; a renewed-contract invoice was paid natively and its schedule line turned `paid`.
- **Documented, NOT changed:** while a `renewed` predecessor is still inside its term and the successor has not started, `edara.unit.occupancy_status` shows `reserved` (occupancy logic only counts `active` contracts, BD-001). Needs a business decision.

### Defect 5 (test fixture)
- `test_branch_code_unique_per_company` created its duplicate against a pre-existing real `RAM` branch (only in the old Community DB). It now creates its own original (`UNIQ1`) first; the constraint itself is untouched.

### Tests added (`tests/test_edara_phase7_hardening.py`, 15)
Unit branch_id (write + web_save blocked, legit field edits work); client-supplied derived values ignored on create; contract company_id/state blocked; manager lifecycle (activate/renew/terminate) still works; Viewer cannot drive lifecycle despite sudo; schedule line amount frozen once invoiced + invoice_id always protected + sibling line unaffected; derived line fields blocked; forged context cannot bypass; su internal writes still allowed; vendor bill currency ILS/USD/JOD; analytic distribution -> analytic line -> payment; no distribution on unrelated bills; renewed contract's remaining periods billed with no gap/duplicate; overlapping renewal drops only successor-covered periods; terminated contracts still not invoiced. NOTE: red/green against pre-fix code was not re-run; the defects themselves were reproduced pre-fix in the Enterprise audit.

### Browser test (REAL browser: Chrome headless via puppeteer-core/CDP against 8070; the Claude-in-Chrome extension was not connected)
Payment Schedule list: 73 rows, buttons Paid/Overdue/Draft present. Paid -> facet "Paid", 3 rows (all Paid). Overdue -> facet "Paid or Overdue", 35 rows (native OR within the same filter group - additive, identical to ticking both in the search dropdown; not a defect). Draft added -> 71 rows. Removing facets -> 73. Overdue alone -> 32, toggled off -> 73. URL/action id unchanged throughout (no navigation, no new action). Lease Contracts and Units lists show no shortcut bar (no global ListView pollution). Browser console: 0 warnings/errors, 0 page errors. Implementation unchanged (`searchModel.toggleSearchItem`). No asset problems.
Tooling note: the Enterprise login page has passkey JS that blocks scripted form submit - the test authenticates over HTTP and injects the `session_id` cookie.

### Live regression (dev DB `odoo19_enterprise_dev`, real JSON-RPC sessions)
Security: RAM manager blocked from branch_id/company_id/state/invoiced amount/invoice_id (incl. forged context); NAB manager blocked cross-branch; Viewer blocked from activate/late fee. Accounting: rent invoice paid natively; deposit collect/deduct/refund, late fee, service charge via manager RPC OK; posted journal balance 0.00; no parallel ledger.

### NEW finding (not fixed, outside Phase 7 scope) - MEDIUM
`edara.deposit.action_deduct` (edara_deposit.py ~l.150) builds the deduction `account.move` with no currency: a USD 200 deduction posts as ILS 200, so for non-company-currency deposits the liability account does not net to zero (observed -540.75 residual after collect 1500 USD / deduct 200 / refund 1300 in the dev DB; the collect/refund payments are correctly multi-currency). Same class as Defect 2.

### Persistent test data added in the dev DB this phase
Maintenance requests "P7 live ILS/USD/JOD" (+3 paid vendor bills); 9 rent invoices for LC/2026/0012 (Jan invoice paid); LC/2026/0009 activated then terminated; USD deposit on LC/2026/0007 (collected/deducted/refunded); 1 late-fee invoice; a service charge on Tower B (600, Unit B01). Earlier audit data unchanged. Test users as listed in the Enterprise Audit section.
...
## Phase 10.1.1 - Yellow Fixes (2026-09-25)

**Status: Implemented, tested (red/green proven), validated on throwaway DBs (real JSON-RPC + HTTP) and on the dev DB. 482/482 automated tests, 0 failed, 0 errors (461 previous + 21 new), on both a fresh install and the upgrade path. Module 19.0.1.2.0.** Fixes exactly the four Phase 10.1 audit findings (Y1-Y4); details in `docs/LEASE_DATE_AND_PRORATION_ENGINE.md` (section 5 "Termination", section 8).

* **Y1 portal renewal:** the form pre-fills and constrains the start date with `contract._term_boundary()` (passed from the controller, not recomputed in QWeb); a start before it is rejected at submission (HTTP 400, message with the earliest date, nothing stored - validated before the INSERT). Model constraint `edara.renewal.request._check_start_after_current_term` backs it for every entry point. Manager approval validation unchanged.
* **Y2 termination:** `edara.lease.contract._truncate_schedule_at(termination_date + 1)` replaces "drop future lines" on termination (the expiry job keeps its own cleanup). Approved rule: preserve earned rent, remove unearned coverage. A crossing uninvoiced line is cut and re-priced by the canonical engine (`_proration_lines`); later uninvoiced lines are removed; earned/overdue lines and lines ending on the boundary are untouched; invoiced lines are never modified (reported on the chatter if they extend past the termination date). The unit can be re-let from `termination_date + 1`. One existing test changed: `test_termination_removes_future_lines_and_stops_cron_invoicing` - the line covering the termination day is now kept (cut to that day) instead of removed, because the termination date is a rent-earning day.
* **Y3 migration:** new `migrations/19.0.1.2.0/post-migration.py` resyncs stored unit occupancy (only where it differs; owner-occupied/sold untouched). The applied 19.0.1.1.0 migration was not edited. Upgrade from 19.0.1.0.0 runs both in order (tested).
* **Y4 guards:** `successor_contract_id` and `predecessor_contract_id` added to the contract's `_edara_system_fields`; `action_renew` sets `predecessor_contract_id` via scoped `sudo()` after creating the successor. Invoiced schedule lines: `due_date`, `period_start`, `period_end`, `period_last_day`, `occupied_days`, `is_prorated` (and `amount`, already) are frozen for non-superusers via `INVOICED_FROZEN_FIELDS`.

### Tests (21 new)
`tests/test_edara_phase1011_fixes.py` (16: termination mid-month / on a boundary / several future lines / quarterly crossing / JOD rounding / invoiced line untouched and reported / re-let next day / idempotent / MAT-FIND-006 earned overdue kept; migration resync + idempotency + upgrade from 1.0.0; guards and frozen dates tested as a **real restricted branch manager** through `write`, `web_save` and a forged `su` context, plus trusted lifecycle actions still working) and 5 in `test_edara_portal.py` (real portal user over HTTP: form default, boundary accepted, day-before rejected with 400, model rejection as a portal user, manager approval). RED: against the old sources 16 of 60 targeted tests failed; the stash/shim was removed and the sources restored byte-identical.

### Validation
* Throwaway DB with a real server, 23/23: restricted manager over JSON-RPC (link fields and invoiced-line dates rejected via write / web_save / forged context; invoiced line unchanged), termination (uninvoiced crossing line cut at today+1 = 366.67 for 11 of 30 days; invoiced line kept and reported), re-let next day, no overlapping periods, portal tenant over HTTP (form default, 400 on day-before, no row stored, boundary accepted, approval -> current active + successor scheduled). Databases dropped.
* Dev DB `odoo19_enterprise_dev`: upgraded 19.0.1.1.0 -> 19.0.1.2.0 (migration ran); unit 8 `reserved` -> `rented` immediately by the migration (no cron); the schedule-line fingerprint (109 lines, 17 invoiced, 246,200.01), the account.move fingerprint (54 moves, 17 posted rent invoices 54,500.00) and payments (20, 40,445.50) are identical before/after; no new invoices; legacy dates and the 44/121 overlap untouched; portal template updated. Rolled-back scenarios 9/9 (termination as a restricted manager, re-let, guards, renewal validation, approval), counts identical afterwards. The portal HTTP flow was exercised on the throwaway server (a dev portal login was not available); on dev only the updated view was verified. The probes consumed more `edara.lease.contract` sequence numbers (a gap, no records).

### Intentionally deferred (unchanged from the Phase 10.1 audit)
`_create_invoice()` state guard; extending expired/renewed contracts; daily repeat of the "scheduled lease could not start" to-do; warning for a renewal starting after the boundary; `period_last_day` silent write; DB exclusion constraint (`btree_gist`); holdover / late-renewal / deposit revaluation / owner settlement; the five old-style contracts and the LC/2026/0012-0015 boundary; the 44/121 overlapping pair; the numbering gap. **Open edge found while fixing Y2:** an *invoiced* period that extends beyond a termination date is not rewritten (accounting review via a chatter note) and, being invoiced, still blocks re-letting inside that period.
